...
本目录包含将已公开的 24 阶 tournament 反例继续压缩的可复现实验代码。搜索失败和求解超时都不等于不存在证明。
...
- 23 阶最小出度为 6 的全部六个分支得到 UNSAT 求解结果；
- 更高出度已完成和仍为 UNKNOWN 的分支逐项记录在 `EXACT_SEARCH_STATUS.json`；
- 两张 23 阶近反例都只剩一个 strong 顶点，保存在 `candidates/`；
...
这些仍是研究阶段结果。只有当 23 阶全部完备分支结束，并且所有 UNSAT 证明由标准检查器复核后，才能声称 24 是最小 tournament 反例阶数。
