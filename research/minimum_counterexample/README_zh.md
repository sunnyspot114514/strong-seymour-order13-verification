...
本目录包含继续压缩当前最小已知 tournament 反例的可复现实验代码。精确搜索
已经在 `d=10,p=4` 分支找到并独立验证了一张 23 阶反例；冻结复现包位于
...
- `d=10,p=4` 分支为 SAT，模型是一张经过验证、strong 顶点数为 0 的
  23 阶反例；
- 当前严格边界为 `14 <= n_min <= 23`；
- 精确最小阶数搜索已经下移到 14 至 22 阶；
...
23 阶 SAT 见证是经过检查的反例，不是由限时运行推断出的结论。只有当某个
